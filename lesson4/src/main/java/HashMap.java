...
public class HashMap<K, V> {
    ...
    /**
     * TODO: Изучить нормально основы Java или умереть
     * 
     * @return
     */
    ...
    public String toString() {
       ...
       StringBuilder ystal = new StringBuilder();
       for (int i = 0; i<buckets.length; i++){
           if (buckets[i] != null && this.buckets[i].head != null){
               ystal.append(buckets[i].head.value.key).append(" - ").append(buckets[i].head.value.value).append("\n");
               Bucket.Node nakonecto = buckets[i].head.next;
               while (nakonecto != null){
                   ystal.append(nakonecto.value.key).append(" - ").append(nakonecto.value.value).append("\n");
                   nakonecto = nakonecto.next;
               }
           }
       }
        return ystal.toString();
    }
